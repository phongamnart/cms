            ...
            <section class="content">
                ...
                <div>
                    <button type="button" class="btn btn-app flat"
                        onclick="setCreate('documents','Document')" title="New">
                                ...
                                <div class="row">
                                    <div class="col-sm-3">
                                            <div class="form-group">
                                                ...
                                                <select class="custom-select"
                                                    style="width: 100%;" <?php echo $mode_select; ?>
                                                    onChange="setFilter('documents_discipline',this.value)">
                                                    <option value="">All</option>
                                                    <?php
												$objQueryDisc = $conDB->sqlQuery($strSQLDisc);
												while($objResultDisc = mysqli_fetch_assoc($objQueryDisc)) { 
													if($objResultDisc['discipline'] == $documents_discipline){$selected = "selected";}else{$selected ="";}
                                    ...
                                    <div class="col-sm-3">
                                            <div class="form-group">
                                                ...
                                                <select class="custom-select"
                                                    style="width: 100%;" <?php echo $mode_select; ?>
                                                    onChange="setFilter('documents_work',this.value)">
                                                    <option value="">All</option>
                                                    <?php
												$objQueryWork = $conDB->sqlQuery($strSQLWork);
												while($objResultWork = mysqli_fetch_assoc($objQueryWork)) { 
													if($objResultWork['work'] == $documents_work){$selected = "selected";}else{$selected ="";}
                                        ...
                                        <div class="col-sm-3">
                                            <div class="form-group">
                                                ...
                                                <select class="custom-select"
                                                    style="width: 100%;" <?php echo $mode_select; ?>
                                                    onChange="setFilter('documents_type',this.value)">
                                                    <option value="">All</option>
                                                    <?php
												$objQueryType = $conDB->sqlQuery($strSQLType);
												while($objResultType = mysqli_fetch_assoc($objQueryType)) { 
													if($objResultType['type'] == $documents_type){$selected = "selected";}else{$selected ="";}
                                    ...
                                    <table id="datatable" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                            <tr
                                                onDblClick="window.location.href='documents_edit.php?no=<?php echo md5($objResult['id']); ?>'">
                                                <td><?php echo $index++; ?></td>
                                                <td align="center">
                                                    <img src="dist/img/icon/edit.svg" onclick="window.location.href='documents_edit.php?no=<?php echo md5($objResult['id']); ?>'"
                                                    title="Edit" width="30" style="padding: 5px;cursor: pointer;"/>
                                                    <?php if($document > 1){?>
                                                    <img src="dist/img/icon/delete.png" onclick="setDelete('documents','<?php echo $objResult['id'];?>','<?php echo $objResult['doc_no'];?>','documents.php')"
                                                    title="Delete" width="30" style="padding: 5px;cursor: pointer;"/>
                                                    <?php }else{ ?>
