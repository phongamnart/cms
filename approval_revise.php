...
<head>
    ...
    while ($obj = mysqli_fetch_assoc($result)) {
        if ($obj['role'] == 'ADMIN' || $obj['role'] == 'ISO') {
            ...
            LEFT JOIN `request` ON `documents`.`doc_no` = `request`.`doc_no` WHERE `request`.`status_rev` = 1 AND `documents`.`enable` = 1" . $condition;
            $objQuery = $conDB->sqlQuery($strSQL);
        } else {
            $strSQL = "SELECT `documents`.*, `request`.*, `documents`.`id` AS `id`, `request`.`id` AS `reqID` FROM `documents`
            LEFT JOIN `request` ON `documents`.`doc_no` = `request`.`doc_no` WHERE `request`.`status_rev` = 999 AND `documents`.`enable` = 1" . $condition;
            $objQuery = $conDB->sqlQuery($strSQL);
        }
    }
        ...
        <div class="content-wrapper">
            <section class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1>Approve Request Revise Documents</h1>
                        </div>
                    </div>
                </div>
            </section>
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                <tr onDblClick="window.open('documents_pdf.php?no=<?php echo md5($objResult['id']); ?>', '_blank');">
                                                    ...
                                                    <td><?php echo $objResult['date_revise'] ?></td>
                                                    <td><?php echo $objResult['createdby'] ?></td>
                                                    <td>
                                                        <?php if ($objResult['status_rev'] == 1) { ?>
                                                            Wait approve
                                                        <?php } elseif ($objResult['status_rev'] == 3) { ?>
                                                            Not approved
                                                        <?php } ?>
                                                    </td>
                                                <?php } ?>
