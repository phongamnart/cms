...
$prev_index = $index_num - 1;

$strSQL = "SELECT * FROM `documents_line_cont` WHERE `index_num` = '$prev_index'";
$objQuery = $conDB->sqlQuery($strSQL);
while ($objResult = mysqli_fetch_assoc($objQuery)) {
    $id_old = $objResult['id'];
}

$str = "UPDATE `documents_line_cont` SET `index_num` = '$prev_index' WHERE `id` = '$id' AND `line_id` = '$line_id'";
$conDB->sqlQuery($str);
